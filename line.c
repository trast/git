...
static int match_funcname(xdemitconf_t *xecfg, char *bol, char *eol)
{
	if (xecfg) {
		char buf[1];
...
static const char *find_funcname_matching_regexp(xdemitconf_t *xecfg, char *start,
						 regex_t *regexp)
{
	int reg_error;
	regmatch_t match[1];
					...
					const char *path)
{
	...
	*end = *begin+1;
	while (*end < lines) {
		char *bol = nth_line_cb(cb_data, *end);
		char *eol = nth_line_cb(cb_data, *end+1);
